...
public class UserService {
    ...
    public UserModel registerUser(UserModel user) throws UserException {
        if (user.getEmail() != null && user.getPassword() != null && user.getUserName() != null) {
            if (userRepo.findByEmail(user.getEmail()).isEmpty()) {
                ...
                return user.builder().userId(entity.getId()).password(entity.getPassword()).email(entity.getEmail())
    ...
    public UserEntity UserModelToUserEntity(UserModel user) {
        UserEntity entity = new UserEntity().builder().email(user.getEmail()).userName(user.getUserName())
                .id(user.getUserId()).password(user.getPassword()).build();
